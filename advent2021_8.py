...
inp = """bceadfg ebcdaf gdaecf fgcaeb fdbea fcdea cbda begdf afb ba | acfbeg cbfgea gebacf ab
...
def parse_input(inp):
    inp = re.findall("(.+) \| (.+)", inp)
    return [ ([set(i) for i in x.split()], [set(i) for i in y.split()]) for x,y in inp]

def seg_to_num(acc, val):
    res = dict(acc)
    res[len(seg[val])].append(val)
    return res

seg = {0: "abcefg", 1: "cf", 2: "acdeg", 3: "acdfg", 4: "bcdf", 5: "abdfg", 6: "abdefg", 7: "acf", 8: "abcdefg", 9: "abcdfg"}
seg_len = {k: len(v) for k, v in seg.items()}
len_segs = reduce(seg_to_num, seg.keys(), {x: [] for x in range(2, 8)})

LENGHTS = {2: 1, 4: 4, 3: 7, 7: 8}

def line_to_nums(line):
    return {LENGHTS[len(i)]: i for i in line if len(i) in LENGHTS}

def find_0(cand, sets):
    return [x for x in cand if len(x & sets[1])==2 and len(x & sets[4])==3 and len(x & sets[7])==3 and len(x & sets[8]) == 6][0]

def find_2(cand, sets):
    return [x for x in cand if len(x & sets[1])==1 and len(x & sets[4])==2 and len(x & sets[7])==2 and len(x & sets[8]) == 5][0]

def find_3(cand, sets):
    return [x for x in cand if len(x & sets[1])==2 and len(x & sets[4])==3 and len(x & sets[7])==3 and len(x & sets[8]) == 5][0]

def find_5(cand, sets):
    return [x for x in cand if len(x & sets[1])==1 and len(x & sets[4])==3 and len(x & sets[7])==2 and len(x & sets[8]) == 5][0]

def find_6(cand, sets):
    return [x for x in cand if len(x & sets[1])==1 and len(x & sets[4])==3 and len(x & sets[7])==2 and len(x & sets[8]) == 6][0]
    
def find_9(cand, sets):
    return [x for x in cand if len(x & sets[1])==2 and len(x & sets[4])==4 and len(x & sets[7])==3 and len(x & sets[8]) == 6][0]

def find_value(val, sets):
    return [ k for k,v in sets.items() if v == val ][0]

def find_number(nums, sets):
    n1, n2, n3, n4 = [find_value(x, sets) for x in nums]
    return n1 * 1000 + n2 * 100 + n3 * 10 + n4

def solve_line(inp):
    s = line_to_nums(inp[0])
    s[0] = find_0(inp[0], s)
    s[2] = find_2(inp[0], s)
    s[3] = find_3(inp[0], s)
    s[5] = find_5(inp[0], s)
    s[6] = find_6(inp[0], s)
    s[9] = find_9(inp[0], s)
    return find_number(inp[1], s)

i = parse_input(inp)
r1 = [y for _,y in i]
r1 = reduce(lambda acc, val: acc + val, r1, [])
r1 = [y for y in r1 if len(y)==seg_len[1] or len(y)==seg_len[4] or len(y)==seg_len[7] or len(y)==seg_len[8]]
print(len(r1))
print(sum([solve_line(x) for x in i]))
